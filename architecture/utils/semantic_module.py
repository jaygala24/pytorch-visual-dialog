...
from architecture.utils import DynamicRNN
...
class SemanticModule(nn.Module):
    def __init__(self, config):
        ...
        self.ques_push = nn.Linear(
            config["lstm_hidden_size"], config["ques_change_num"]
        )

        self.caption_push = nn.Linear(
            config["lstm_hidden_size"], config["caption_change_num"]
        )
    ...
    def forward(self, caption, lencap, cap_numrounds, ques, global_cap):
        ...
        caption = caption.repeat(1, numrounds, 1).view(
            int(batch * numrounds), cap_numrounds, -1
        )
        ques = ques.repeat(1, 1, cap_numrounds).view(
            int(batch * numrounds), -1, ques_emb_size
        )
    ...
    def _cat_gl_cap(self, global_cap, local_cap):
        ...
        for i in range(global_cap.size(0)):
            if i == 0:
                ...
                new_cap = torch.cat((new_cap, global_cap[i], local_cap[i]), 0).view(
                    -1, dim
                )
